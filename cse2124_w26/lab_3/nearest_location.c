...
int main() {
    // Given locations
    double lat1 = 25.778, lon1 = 88.897;  // Saidpur
    double lat2 = 25.513, lon2 = 89.856;  // Boraibari
    double lat3 = 25.286, lon3 = 89.863;  // Kamalpur
    ...
    double d1 = (lat - lat1)*(lat - lat1) + (lon - lon1)*(lon - lon1);
    double d2 = (lat - lat2)*(lat - lat2) + (lon - lon2)*(lon - lon2);
    double d3 = (lat - lat3)*(lat - lat3) + (lon - lon3)*(lon - lon3);
